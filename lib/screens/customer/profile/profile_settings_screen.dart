...
class _ProfileSettingsScreenState extends ConsumerState<ProfileSettingsScreen> {
      ...
      builder: (context) => SafeArea(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(title, style: AppText.serif(fontSize: 23, color: AppColors.clientText(context))),
            const SizedBox(height: 10),
            for (final option in options) ListTile(title: Text(option, style: AppText.sans(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.clientText(context))), trailing: option == value ? Icon(Icons.check_rounded, color: AppColors.clientTealText(context)) : null, onTap: () { onChanged(option); Navigator.pop(context); }),
          ]),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    body: ClientBackground(
      ...
      Text(clientLabel('PREFERENCES', 'MAPENDEKEZO', language), style: AppText.eyebrow(color: AppColors.clientSecondaryText(context))),
      ...
      Text(clientLabel('LEGAL', 'KISHERIA', language), style: AppText.eyebrow(color: AppColors.clientSecondaryText(context))),
  ...
  void _showDocument(BuildContext context, String title, String body) => showModalBottomSheet<void>(
    ...
    builder: (context) => Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: AppText.serif(fontSize: 24, color: AppColors.clientText(context))),
        const SizedBox(height: 14),
        Text(body, style: AppText.sans(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.clientSecondaryText(context), height: 1.55)),
      ]),
    ),
  );
}
...
class _SettingRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 4), leading: Icon(icon, color: AppColors.clientTealText(context)), title: Text(title, style: AppText.sans(fontSize: 14, fontWeight: FontWeight.w800, color: AppColors.clientText(context))), trailing: Row(mainAxisSize: MainAxisSize.min, children: [if (value != null) Text(value!, style: AppText.sans(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.clientSecondaryText(context))), const SizedBox(width: 8), Icon(Icons.chevron_right_rounded, color: AppColors.clientMutedIcon(context))]), onTap: onTap);
}
